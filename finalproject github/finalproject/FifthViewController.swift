...
class FifthViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func goToArticle(_ sender: Any) {
        if let url = URL(string: "https://www.theverge.com/2019/4/29/18522637/burger-king-impossible-whopper-nationwide-rollout-meatless-vegetarian") {
    ...
    @IBAction func addPost(_ sender: Any) {
        ...
        ref?.child("Science").childByAutoId().setValue(textView.text)
    ...
    override func viewDidLoad() {
        ...
        databaseHandle =
            ref?.child("Science").observe(.childAdded, with: { (snapshot) in
