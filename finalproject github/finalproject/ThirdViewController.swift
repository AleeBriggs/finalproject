...
class ThirdViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func goToArticle(_ sender: Any) {
        if let url = URL(string: "https://nba.nbcsports.com/2019/04/29/spurs-staffer-reportedly-says-no-doubt-gregg-popovich-will-return/") {
    ...
    @IBAction func addPost(_ sender: Any) {
        ...
        ref?.child("Sports").childByAutoId().setValue(textView.text)
    ...
    override func viewDidLoad() {
        ...
        databaseHandle =
        ref?.child("Sports").observe(.childAdded, with: { (snapshot) in
